...
class VotingEngine:
    ...
    def submit_votes(self, voter_adm: str,
        ...
        for committee, candidate_adm in votes_dict.items():
            ...
            valid, msg = self.verify_vote_integrity(voter_adm, candidate_adm, db_committee)
            if not valid:
                return False, f"{db_committee}: {msg}"

        actual = {c.replace('House-', ''): a
                  for c, a in votes_dict.items() if a != '__abstain__'}

        token = self._token.get_unused(voter_adm) or self._token.generate_token(voter_adm)
        if not token:
    ...
    def get_student_votes(self, voter_adm: str) -> Dict[str, Dict]:
        ...
        result = {}
        for committee, candidate_adm, created_at in self._vote.get_student_votes(voter_adm):
    ...
    def get_results(self) -> Dict:
        ...
        for row in rows:
            ...
            results[ctype].setdefault(cname, {
                'total_votes': 0,
