...
def test_excel_validation():
    ...
    try:
        ...
        new_classes = sorted(list(set(valid_classes) | excel_classes), key=lambda x: int(x) if x.isdigit() else 999)
        ...
        for index, row in df.iterrows():
            ...
            is_valid, error_msg, cleaned_data = Utils.validate_student_data(row_dict, db)
            if not is_valid:
