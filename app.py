...
# PAGE CONFIG
# ─────────────────────────────────────────────────────────────────────────────
st.set_page_config(page_title='JB Academy Elections', page_icon='🗳️',
...
DEFAULT_HOUSE = {'color':'#6366f1','bg':'rgba(99,102,241,0.12)','border':'rgba(99,102,241,0.4)','emoji':'🏠','icon':'🏠'}

COMMITTEE_INFO = {
    'Sports':      {'icon':'⚽','desc':'Organises sports days, tournaments & inter-house competitions'},
    'Literary':    {'icon':'📖','desc':'Runs debates, quiz bowls, school magazine & reading clubs'},
    'Eco':         {'icon':'🌱','desc':'Champions green initiatives, recycling & environment drives'},
    'Cultural':    {'icon':'🎭','desc':'Plans cultural fests, art shows, music & drama events'},
    'Maintenance': {'icon':'🔧','desc':'Keeps classrooms, labs & common areas clean and functional'},
    'Discipline':  {'icon':'⚖️','desc':'Upholds school rules, resolves conflicts & promotes respect'},
    'CCA':         {'icon':'🎨','desc':'Co-curricular activities — clubs, hobby groups & talent shows'},
}
...
def avatar(name: str) -> str:
    random.seed(str(name)); return random.choice(AVATARS)

def hm(house: str) -> dict:
    return HOUSE_META.get(house, DEFAULT_HOUSE)

def ci(committee: str) -> dict:
    return COMMITTEE_INFO.get(committee, {'icon':'📋','desc':'Serves the school community.'})
...
.stButton>button{border-radius:12px!important;font-weight:600!important;transition:all .2s ease!important;letter-spacing:.2px!important}
...
def confetti_html() -> str:
    colors = ['#6366f1','#8b5cf6','#ec4899','#10b981','#f59e0b','#3b82f6','#ef4444','#34d399']
    pieces = []
    for _ in range(60):
...
# ─────────────────────────────────────────────────────────────────────────────
_defaults = {
    'user_type':None,'user_data':None,'review_votes':False,'temp_votes':{},
...
if st.session_state.user_type is None:
    ...
    with mid:
        ...
        lid  = st.text_input('Admission No / Admin ID', placeholder='e.g. JB001', key='lid')
        lpwd = st.text_input('Password', type='password', placeholder='Your password', key='lpwd')

        if st.button('🔓  Sign In', use_container_width=True, type='primary'):
            if not lid or not lpwd:
                st.markdown('<div class="error-box">❌ Please enter both ID and password.</div>', unsafe_allow_html=True)
            else:
                ...
                if res['type'] == 'admin':
                    ...
                    st.markdown(f'<div class="error-box">🔒 {res["message"]}</div>', unsafe_allow_html=True)
                else:
                    st.markdown(f'<div class="error-box">❌ {res["message"]}</div>', unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with ah1:
        st.markdown('<div style="font-size:1.1rem;font-weight:700;color:#94a3b8;">🛠️ Administrator Dashboard</div>', unsafe_allow_html=True)
    with ah2:
    ...
    with t1:
        ...
        f = st.file_uploader('📂 Upload Excel (.xlsx)', type=['xlsx'], label_visibility='hidden')
        if f:
            try:
                ...
                if imp > 0:
                    ...
                    st.download_button(
                        '📥 Download Password List', data=pwd_file,
                ...
                st.markdown(f'<div class="error-box">❌ {ex}</div>', unsafe_allow_html=True)

    # ── TAB 2: STUDENTS ───────────────────────────────────────────────────────
    with t2:
        if st.button('📥 Download All Passwords', use_container_width=True, type='primary'):
            pwd_file = Utils.create_password_file(Student(db).get_all())
            st.download_button(
                '📥 Download Now', data=pwd_file,
                file_name=f'passwords_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}.xlsx',
                mime='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',
        ...
        with c1: srch = st.text_input('🔍 Search', placeholder='Name or Adm No').strip().lower()
        with c2: fcls = st.selectbox('Class', ['All', '7', '8', '9', '10', '11', '12'])
        with c3: fhs  = st.selectbox('House', ['All'] + voting.HOUSES)

        all_s = Student(db).get_all()
        df_s  = pd.DataFrame(all_s, columns=['Adm','Name','Class','Sec','House','Hash','Pwd','Voted','Crt','Upd'])
        if srch:
            df_s = df_s[df_s['Name'].str.lower().str.contains(srch, na=False) |
                        df_s['Adm'].str.lower().str.contains(srch, na=False)]
        if fcls != 'All': df_s = df_s[df_s['Class'] == fcls]
        if fhs  != 'All': df_s = df_s[df_s['House'] == fhs]
        ...
        st.dataframe(df_s[['Adm','Name','Class','Sec','House','Pwd','Voted']],
        ...
        if st.session_state.show_add:
            ...
            if st.button('✅ Add Student', type='primary', use_container_width=True):
                if not na or not nn:
                    st.markdown('<div class="error-box">❌ Admission No and Name are required.</div>', unsafe_allow_html=True)
                else:
                    pwd = Utils.generate_password()
                    if Student(db).add(na, nn, nc, ns, nh, Auth.hash_password(pwd), pwd):
                        st.markdown(
                        ...
                        st.markdown('<div class="error-box">❌ Failed — admission number may already exist.</div>', unsafe_allow_html=True)

        if st.session_state.show_edit:
            st.markdown("#### ✏️ Edit Student")
            sl = Student(db).get_all()
            if sl:
                ea = st.selectbox('Select', [s[0] for s in sl],
                ...
                if s:
                    ...
                    with c1:
                        ...
                        ec = st.selectbox('Class', ['7','8','9','10','11','12'],
                                          index=['7','8','9','10','11','12'].index(s[2]) if s[2] in ['7','8','9','10','11','12'] else 0)
                    with c2:
                        es = st.selectbox('Section', ['A','B','C','D','E'],
                                          index=['A','B','C','D','E'].index(s[3]) if s[3] in ['A','B','C','D','E'] else 0)
                        eh = st.selectbox('House', voting.HOUSES,
                                          index=voting.HOUSES.index(s[4]) if s[4] in voting.HOUSES else 0)
        ...
        if st.session_state.show_reset:
            ...
            sl = Student(db).get_all()
            if sl:
                ra = st.selectbox('Select', [s[0] for s in sl],
                                  format_func=lambda x: f"{x} — {next((s[1] for s in sl if s[0]==x), x)}",
                                  key='ra')
        ...
        if st.session_state.show_del:
            ...
            sl = Student(db).get_all()
            if sl:
                da = st.selectbox('Select', [s[0] for s in sl],
                                  format_func=lambda x: f"{x} — {next((s[1] for s in sl if s[0]==x), x)}",
                                  key='da')
                s = Student(db).get(da)
                if s:
                    st.markdown(f'<div class="warn-box">⚠️ Delete <strong>{s[1]}</strong> ({da})?</div>', unsafe_allow_html=True)
                    if s[7] == 1:
                        st.markdown('<div class="error-box">❌ Cannot delete — student has already voted.</div>', unsafe_allow_html=True)
                        ...
                        if st.button('🗑️ Confirm Delete', type='secondary', use_container_width=True):
                            if Student(db).delete(da):
                                st.markdown(f'<div class="success-box">✅ Deleted {s[1]}</div>', unsafe_allow_html=True)
        ...
        with st.expander('🔐 Change Admin Password'):
            ...
            if st.button('Update Admin Password', type='primary'):
                if len(cp1) < 8:
                    st.markdown('<div class="error-box">❌ Password must be at least 8 characters.</div>', unsafe_allow_html=True)
                elif cp1 != cp2:
                    st.markdown('<div class="error-box">❌ Passwords do not match.</div>', unsafe_allow_html=True)
                else:
                    if auth.set_admin_password(cp1):
                        st.markdown('<div class="success-box">✅ Admin password updated. Please log in again.</div>', unsafe_allow_html=True)
    ...
    with t3:
        ...
        with cs1:
            ...
            for ctype in ['School', 'House']:
                ...
                if not rows:
                    continue
                st.markdown(f"**{'🏫 School' if ctype=='School' else '🏠 House'} Committees**")
                for r in rows:
                    info = ci(r[1])
    ...
    with t4:
        ...
        with ns1:
            ...
            if not pending:
                st.markdown('<div class="success-box">🎉 All nominations reviewed — nothing pending!</div>', unsafe_allow_html=True)
            else:
                st.markdown(f'<div class="warn-box">⏳ <strong>{len(pending)}</strong> nomination(s) need your review.</div>', unsafe_allow_html=True)
                for row in pending:
                    cid, adm, name, ctype, comm, sc, sh, sg, manifesto, status, nom_by, created = row
                    st.markdown(f"""
                    ...
                    with ac1:
                        if st.button('✅ Approve', key=f'app_{cid}', type='primary', use_container_width=True):
                            Candidate(db).approve(cid)
                            audit.log('APPROVE_NOM', 'admin', f'{name}→{comm}')
                            st.rerun()
                    with ac2:
                        if st.button('❌ Reject', key=f'rej_{cid}', type='secondary', use_container_width=True):
        ...
        with ns2:
            if live_now:
                st.markdown('<div class="warn-box">⚠️ Election is live — nominations are locked. Stop the election to add nominations.</div>', unsafe_allow_html=True)
            else:
                c1, c2 = st.columns(2)
                with c1:
                    ...
                    if nom_type == 'School':
                        ...
                        nom_cls  = st.selectbox('Class', ['7','8','9','10','11','12'])
                        nom_hs = nom_grp = None
                    else:
                        nom_comm = st.selectbox('Committee', hc_l) if hc_l else None
                        nom_hs   = st.selectbox('House', voting.HOUSES)
                        nom_grp  = st.selectbox('Group', ['Junior (7-8)', 'Senior (9-12)']).split('(')[0].strip()
                        nom_cls  = None
                ...
                with c2:
                    if nom_adm:
                        ...
                        if cp:
                            ...
                            st.markdown('<div class="warn-box">⚠️ Student not found</div>', unsafe_allow_html=True)

                if st.button('✅ Nominate', type='primary', use_container_width=True):
                    if not nom_comm:
                        st.markdown('<div class="error-box">❌ No committees available.</div>', unsafe_allow_html=True)
                    else:
                        cand = Student(db).get(nom_adm)
                        if not cand:
                            st.markdown(f'<div class="error-box">❌ Student {nom_adm} not found.</div>', unsafe_allow_html=True)
                        else:
                            ok, msg = Candidate(db).add(nom_adm, nom_type, nom_comm, nom_cls, nom_hs, nom_grp, nom_man, 'approved', 'admin')
                            if ok:
                                st.markdown(f'<div class="success-box">✅ {cand[1]} nominated for {nom_comm}.</div>', unsafe_allow_html=True)
        ...
        with ns4:
            ...
            if not all_noms:
                ...
                for ctype in ['School', 'House']:
                    ...
                    if not rows:
                        continue
                    st.markdown(f"**{'🏫 School' if ctype=='School' else '🏠 House'}**")
                    for row in rows:
                        cid, adm, name, ct, comm, sc, sh, sg, manifesto, status, nom_by, created = row
    ...
    with t6:
        ...
        with rs1:
            ...
            if not results_data:
                st.markdown('<div class="info-box">📋 No results yet — nominations and votes needed first.</div>', unsafe_allow_html=True)
            else:
                for ctype in ['School', 'House']:
                    ...
                    for comm_name, data in sorted(results_data[ctype].items()):
                        ...
                        for i, c_ in enumerate(cands):
                            ...
                            rank_ico = RANK_ICONS[i] if i < len(RANK_ICONS) else str(i + 1)
        ...
        with rs2:
            ...
            sc1.metric('Total Students', stats['total_students'])
            sc2.metric('Participated', f"{stats['voted_students']} ({stats['participation_rate']:.1f}%)")
            ...
            if stats.get('votes_by_committee'):
                st.markdown('<br>**Votes by Committee**')
                df_vc = pd.DataFrame(
        ...
        with rs3:
            logs = audit.get_all()
            if logs:
                df_l = pd.DataFrame(logs, columns=['ID','Action','User','Details','IP','Timestamp'])
                df_l['Timestamp'] = df_l['Timestamp'].apply(Utils.format_timestamp)
                st.dataframe(df_l[['ID','Action','User','Details','Timestamp']],
                             use_container_width=True, hide_index=True)
                st.download_button(
                    '📥 Export Audit Log (CSV)',
                    data=df_l[['ID','Action','User','Details','Timestamp']].to_csv(index=False),
    ...
    with t8:
        ...
        if votes:
            ...
            df_v = pd.DataFrame(votes, columns=['ID','Cand Adm','Committee','Timestamp'])
            ...
            st.download_button(
                '📥 Export Votes CSV',
                data=df_v[['ID','Candidate','Committee','Timestamp']].to_csv(index=False),
                file_name=f'votes_{pd.Timestamp.now().strftime("%Y%m%d_%H%M%S")}.csv'
            )
            st.divider()
            st.markdown("#### 🗑️ Delete a Vote (Admin Override)")
            st.markdown('<div class="warn-box">⚠️ Permanently logged. A reason is required.</div>', unsafe_allow_html=True)
            ...
            if st.button('Delete Vote', type='secondary'):
                if not dv_r.strip():
                    st.markdown('<div class="error-box">❌ Reason required.</div>', unsafe_allow_html=True)
                elif Vote(db).delete_vote(int(dv_id)):
                    audit.log('DELETE_VOTE', 'admin', f'ID {dv_id}: {dv_r}')
                    st.markdown('<div class="success-box">✅ Deleted and logged.</div>', unsafe_allow_html=True)
                    st.rerun()
                else:
                    st.markdown('<div class="error-box">❌ Vote ID not found.</div>', unsafe_allow_html=True)
        else:
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if fresh[7] == 1:
        ...
        if my_votes:
            st.markdown("**Your selections (read-only):**")
        ...
        st.markdown(f'<div class="civic-fact" style="margin-top:24px;">💡 {fact}</div>', unsafe_allow_html=True)
        st.stop()
    ...
    with stab2:
        if not voting.nominations_open():
            ...
            # Nomination form
            nom_type_s = st.selectbox('I want to stand for a', ['School Committee', 'House Committee'])
            ...
            if is_school:
                ...
                if nom_comm_s:
                    info = ci(nom_comm_s)
                    st.markdown(f'<div class="info-box">{info["icon"]} <strong>{nom_comm_s}</strong> — {info["desc"]}</div>', unsafe_allow_html=True)
            else:
                hc_l       = voting.HOUSE_COMMITTEES
            ...
            st.caption(f"{len(nom_man_s)}/200 characters · {200 - len(nom_man_s)} remaining")

            if st.button('✋ Submit My Nomination', type='primary', use_container_width=True):
                if not nom_comm_s:
                    st.markdown('<div class="error-box">❌ No committees available yet.</div>', unsafe_allow_html=True)
                elif len(nom_man_s.strip()) < 20:
                    st.markdown('<div class="warn-box">✍️ Please write at least 20 characters.</div>', unsafe_allow_html=True)
                else:
                    ok, msg = Candidate(db).add(
                        fresh[0], 'School' if is_school else 'House',
                        nom_comm_s, nom_cls_s, nom_hs_s, nom_grp_s,
                        nom_man_s.strip(), 'pending', fresh[0]
                    )
                    if ok:
                        audit.log('SELF_NOMINATION', fresh[0], f'{fresh[1]}→{nom_comm_s}')
                        st.markdown("""
    ...
    with stab1:
        ...
        if not st.session_state.review_votes:
            ...
            def render_committee(comm_key, cands_rows, label_override=None):
                ...
                data_map = {r[1]: {'adm': r[1], 'name': r[3] or r[1], 'class': r[4] or '?',
                                   'sec': r[5] or '?', 'house': r[6] or '?',
                                   'manifesto': r[2] or ''} for r in cands_rows}
                ...
                for idx, opt_key in enumerate(options):
                    ...
                    with col:
                        ...
                        if opt_key == ABSTAIN:
                            ...
                            d  = data_map[opt_key]
                            ch = hm(d['house'])
                            check = '<div class="cand-selected-check">✓</div>' if is_sel else ''
                            manifesto_html = f'<div class="cand-manifesto">"{d["manifesto"]}"</div>' if d['manifesto'] else ''
                            st.markdown(f"""
            ...
            for comm in voting.HOUSE_COMMITTEES:
                if house_by_comm.get(comm):
                    if not house_shown:
                        st.markdown(f"## {h['emoji']} {fresh[4]} House Committees")
                        house_shown = True
                    vote_choices = render_committee(
                        f'House-{comm}', house_by_comm[comm],
            ...
            if not school_shown and not house_shown:
                ...
                with c1:
                    if st.button('👁️  Review My Votes →', use_container_width=True, type='primary'):
                        st.session_state.temp_votes  = vote_choices
        ...
        # ── STEP 2: REVIEW & CONFIRM ──────────────────────────────────────────
            ...
            if real_votes == 0:
                st.markdown('<div class="warn-box">⚠️ You have abstained from all committees. Are you sure?</div>', unsafe_allow_html=True)

            st.markdown('<br>', unsafe_allow_html=True)
            c1, c2 = st.columns(2)
            with c1:
                if st.button('✅  CONFIRM & SUBMIT MY VOTE', use_container_width=True, type='primary'):
