...
students = cursor.execute(
    "SELECT admission_no, class, house FROM students LIMIT 20"
).fetchall()
...
if len(committees) == 0:
    ...
    conn.commit()
    committees = cursor.execute(
        "SELECT name, committee_type FROM committees"
    ).fetchall()
        ...
        try:
            cursor.execute(
                ...
                (admission_no, committee_type, committee_name, scope_class,
                 scope_house, section_group, manifesto, status, nominated_by,
                 created_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
                (
                    ...
                    student_house,
                    None,
                    f"Test manifesto for {comm_name}",
                    "approved",
                    "admin",
