...
def _render_house_participation_chart(cands: list, comm_name: str):
    ...
    # Prepare data
    df = pd.DataFrame([
        {'House': house, 'Votes': votes}
        for house, votes in house_votes.items()
    ])
...
def _render_animated_podium(cands: list, comm_name: str):
    ...
    for pi, ri in enumerate(order):
        if ri < len(top3):
            ...
            delay = f"animation-delay: {pi * 0.2}s;"
            
            with pcols[pi]:
                st.markdown(f"""
                <div class="{animation_class}" style="{delay}text-align:center;">
...
def render_results(results: Dict, show_tie_break: bool = False,
                   on_tie_break=None, student_votes: Dict = None):
    """
    ...
    for ctype in ['School', 'House']:
        ...
        for comm_name, data in sorted(results[ctype].items()):
            ...
            # Full ranked list
            student_voted_for = student_votes.get(comm_name, {}).get('candidate_name', '') if student_votes else ''
            for i, c_ in enumerate(cands):
                is_student_choice = (student_voted_for and c_['name'] == student_voted_for)
                st.markdown(result_card_html(i + 1, c_, max_w, total_cv, is_student_choice),
                            unsafe_allow_html=True)
            st.markdown('<br>', unsafe_allow_html=True)
