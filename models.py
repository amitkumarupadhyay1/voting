...
class Student:
    ...
    def get_all(self) -> List[Tuple]:
        return [tuple(r) for r in
                self.db.execute('SELECT * FROM students ORDER BY admission_no').fetchall()]
    ...
    def update(self, admission_no: str, name: str = None, class_num: str = None,
        ...
        if class_num: updates.append('class=?');   params.append(class_num)
        if section:   updates.append('section=?'); params.append(section)
        if house:     updates.append('house=?');   params.append(house)
        if not updates:
            return False
    ...
    def reset_password(self, admission_no: str, password_hash: str,
        ...
        return self.db.write_many([
            ('''UPDATE students SET password=?,generated_password=?,updated_at=?
                WHERE admission_no=?''',
             (password_hash, password_plain, datetime.now().isoformat(), admission_no))
        ])
...
class Vote:
    ...
    def cast_all(self, vote_token: str, admission_no: str,
        ...
        stmts.append(('UPDATE students SET has_voted=1,updated_at=? WHERE admission_no=?',
                      (now, admission_no)))
        ...
        with Database._write_lock:
            ...
            try:
                ...
                row = conn.execute(
                    'SELECT has_voted FROM students WHERE admission_no=?', (admission_no,)
                ).fetchone()
...
class Candidate:
    ...
    def add(self, admission_no: str, committee_type: str, committee_name: str,
            ...
            section_group: str = None, manifesto: str = None,
            status: str = 'approved', nominated_by: str = 'admin') -> Tuple[bool, str]:
        ...
        ok = self.db.write_many([(
            '''INSERT INTO candidates
               ...
               VALUES (?,?,?,?,?,?,?,?,?,?)''',
            (adm, committee_type, committee_name, scope_class, scope_house,
             section_group, manifesto or '', status, nominated_by,
             datetime.now().isoformat())
        )])
...
class Committee:
    DEFAULT_SCHOOL = ['Sports', 'Literary', 'Eco', 'Cultural', 'Maintenance', 'Discipline']
...
class Election:
    ...
    def get_statistics(self) -> Dict:
        ...
        voted   = self.db.execute('SELECT COUNT(*) FROM students WHERE has_voted=1').fetchone()[0]
...
class AuditLog:
    ...
    def log(self, action: str, user_adm: str = None,
    ...
    def get_all(self) -> List[Tuple]:
        return [tuple(r) for r in
                self.db.execute('SELECT * FROM audit_log ORDER BY created_at DESC').fetchall()]
